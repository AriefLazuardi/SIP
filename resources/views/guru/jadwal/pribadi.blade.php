...
<div class="container mx-auto p-4 bg-white mt-8">
    ...
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Jadwal Mata Pelajaran Pribadi</h2>
        <div class="flex items-center gap-2">
            <div x-data="{ search: '{{ request('search_pribadi') }}' }" class="flex items-center border border-customColor rounded-xl p-1">
                <span class="material-icons text-primaryColor ml-2">search</span>
                <form action="{{ route('guru.jadwal.pribadi') }}" method="GET">
                    <input 
                        type="text" 
                        name="search_pribadi" 
                        placeholder="Cari" 
                        class="flex-grow border-0 focus:ring-0" 
                        x-model="search_pribadi"
                    >
                </form>
            </div>
            <div class="w-12 h-12 items-center flex justify-center rounded-md border-primaryColor border-2">
                <a href="{{ route('cetak.jadwal-guru') }}" class="w-12 h-12 items-center flex justify-center rounded-md">
    ...
    <form action="{{ route('guru.jadwal.pribadi') }}" method="GET" class="mb-4">
        @if ($selectedPeriod)
        ...
        <div class="flex space-x-2 mb-2">
            @foreach ($tingkatanKelasGuru as $tingkat)
                <button type="submit" 
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <div class="flex space-x-2">
                @foreach (['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat'] as $index => $hari)
                    @if($hariDiajar->contains($index + 1))
                        <button type="submit" 
                            name="hari_id" 
                            value="{{ $index + 1 }}"
            ...
            <div class="bg-primaryColor w-28 h-12 border-2 rounded-md flex justify-center items-center">
                <span class="font-semibold text-whiteColor text-center">
                    Sesi {{ $jadwalMatrix ? array_values($jadwalMatrix)[0]['sesi_belajar'] : 'Tidak Ada' }}
                </span>
            </div>
        </div>
    </form>
