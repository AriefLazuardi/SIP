<section class="space-y-6">
    <header>
        ...
        <p class="mt-1 text-sm text-customColor">
    ...
    <x-danger-button x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion')" class="justify-center w-1/2">
        {{ __('Hapus Akun') }}
    </x-danger-button>
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
            ...
            <div class="mt-6 flex justify-center space-x-8">
                <x-secondary-button x-on:click="$dispatch('close')" class="py-2 justify-center w-1/2">
                    {{ __('Batal') }}
                </x-secondary-button>

                <x-danger-button class="py-2 justify-center w-1/2">
                    {{ __('Hapus') }}
                </x-danger-button>
            </div>
        </form>
